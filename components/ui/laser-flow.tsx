...
const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const lowerThresh = 50, upperThresh = 58, dprFloor = 0.6
    ...
    const adjustDpr = (now: number) => {
      ...
      if (avg < lowerThresh) next = clamp(currentDprRef.current * 0.9, dprFloor, base)
      else if (avg > upperThresh && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.05, dprFloor, base)
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || '#A2F24B');
